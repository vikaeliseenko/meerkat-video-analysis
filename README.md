...
Проект содержит полный воспроизводимый пакет для автоматизированного анализа видео `IMG_1561.MP4`: подготовку кадров, первичную разметку, дообучение детектора сурикатов, трекинг, классификацию поведенческих событий, графики, таблицы и текстовые фрагменты для работы.

Важно: модель обучена на автоматически полученной первичной разметки из небольшого количества видеоматериала. Результаты подходят как рабочий прототип и предварительный анализ, но для строгих научных выводов нужна более детальная экспертная валидация разметки, зоны обогащения и поведенческих категорий по большому количеству видеоматериалов.

## Структура

- `videos/IMG_1561.MP4` - исходное видео.
- `src/00_extract_frames.py` - извлечение кадров и обзорных листов.
- `src/01_generate_pseudo_labels.py` - автоматическая первичная разметка в формате YOLO.
